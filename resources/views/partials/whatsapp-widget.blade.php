<div class="fixed right-4 bottom-4 z-[70] flex flex-col items-end gap-3 sm:right-6 sm:bottom-6">
    ...
    <div data-wa-panel class="hidden w-[min(340px,calc(100vw-32px))] animate-pop-in glass-strong rounded-[26px] p-5">
        <div class="mb-4 flex items-start gap-3">
            <div class="flex-1">
                <div class="text-[17px] font-semibold" style="font-family:var(--font-heading)">Message us on WhatsApp</div>
                <div class="mt-1 text-[12.5px] text-ink/45">Pick the desk closest to you</div>
            </div>
            <button data-wa-close type="button" aria-label="Close" class="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-black/5 text-[13px] text-ink/60">✕</button>
        </div>
                ...
                <a href="{{ whatsapp_link($contact['number']) }}" target="_blank" rel="noopener"
                   class="flex items-center gap-3 rounded-2xl border border-white/80 bg-white/72 p-3.5 transition hover:-translate-y-0.5 hover:bg-white">
                    ...
                    <span class="min-w-0 flex-1">
                        ...
                        <span class="block text-[11.5px] text-ink/45">{{ $contact['role'] }} · {{ $contact['country'] }}</span>
        ...
        <p class="mt-3.5 text-[11.5px] text-ink/45">{{ config('travel.brand.hours') }}</p>
    </div>

    <button data-wa-toggle type="button" aria-label="WhatsApp us" aria-expanded="false"
