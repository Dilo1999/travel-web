...
<article class="glass-strong flex flex-col overflow-hidden rounded-[28px] transition-transform duration-300 hover:-translate-y-2">
    ...
    <div class="flex flex-1 flex-col p-6">
        <h3 class="mb-2.5 text-xl">{{ $package['title'] }}</h3>
        <p class="mb-4 flex-1 text-[13.5px] leading-relaxed text-ink/60">{{ $package['blurb'] }}</p>
        <div class="mb-5 flex flex-wrap gap-2">
            <span class="rounded-full bg-black/5 px-3.5 py-1.5 text-xs font-semibold">{{ $package['days'] }} days</span>
            <span class="rounded-full bg-black/5 px-3.5 py-1.5 text-xs font-semibold">{{ $package['pax'] }}</span>
        </div>
        <div class="flex flex-wrap gap-2.5">
            <a href="{{ route('packages.show', $package['slug']) }}" class="btn btn-primary px-4.5 py-3 text-[12.5px]">Itinerary</a>
            <button type="button" data-wa-open class="btn btn-secondary px-4.5 py-3 text-[12.5px]">WhatsApp</button>
        </div>
    </div>
</article>
