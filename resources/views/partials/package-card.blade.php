...
<article class="glass-strong flex flex-col overflow-hidden rounded-[22px] transition-transform duration-300 hover:-translate-y-2 sm:rounded-[28px]">
    <a href="{{ route('packages.show', $package['slug']) }}" class="relative block h-[200px] bg-[#dfe5de] bg-cover bg-center sm:h-[210px]" style="background-image:url('{{ $package->heroImageUrl(1000, 700) }}')">
        ...
        <span class="absolute bottom-3.5 left-4 rounded-full bg-[rgba(10,24,14,.5)] px-3 py-1.5 text-xs font-semibold text-white backdrop-blur-md">
            {{ travel_t($package['location']) }}
        </span>
    </a>
    <div class="flex flex-1 flex-col p-5 sm:p-6">
        <h3 class="mb-2 text-lg sm:mb-2.5 sm:text-xl">{{ travel_t($package['title']) }}</h3>
        <p class="mb-4 flex-1 text-[13.5px] leading-relaxed text-ink/60 max-sm:line-clamp-3">{{ travel_t($package['blurb']) }}</p>
        <div class="mb-5 flex flex-wrap gap-2">
            <span class="rounded-full bg-black/5 px-3.5 py-1.5 text-xs font-semibold">{{ $package['days'] }} {{ __('site.common.days') }}</span>
            <span class="rounded-full bg-black/5 px-3.5 py-1.5 text-xs font-semibold">{{ travel_t($package['pax']) }}</span>
        </div>
        <div class="flex flex-wrap gap-2.5 max-sm:grid max-sm:grid-cols-2">
