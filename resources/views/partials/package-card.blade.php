...
<article class="glass-strong flex flex-col overflow-hidden rounded-[22px] transition-transform duration-300 hover:-translate-y-2 sm:rounded-[28px]">
    <a href="{{ route('packages.show', $package['slug']) }}" class="relative block h-[200px] bg-[#dfe5de] bg-cover bg-center sm:h-[210px]" style="background-image:url('{{ travel_img($package['img'], 1000, 700, travel_place($package['where']).','.$package['country']) }}')">
