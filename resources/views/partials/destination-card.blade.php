...
<article class="glass-strong flex flex-col overflow-hidden rounded-[28px] transition-transform duration-300 hover:-translate-y-2">
    ...
    <div class="flex flex-1 flex-col p-6">
        <h3 class="mb-1 text-[22px]">{{ $destination['name'] }}</h3>
        <div class="mb-3 text-[12.5px] text-ink/45">{{ $destination['country'] }}</div>
        <p class="mb-4 flex-1 text-[13.5px] leading-relaxed text-ink/60">{{ $destination['blurb'] }}</p>
        <div class="mb-4.5 flex flex-wrap gap-2">
            @foreach ($destination['tags'] as $tag)
                <span class="rounded-full bg-accent-100 px-3 py-1.5 text-[11.5px] font-semibold text-accent-800">{{ $tag }}</span>
            @endforeach
        </div>
        <a href="{{ route('packages.index', ['kind' => $destination['kind']]) }}" class="btn btn-primary self-start px-4.5 py-3 text-[12.5px]">See packages</a>
    </div>
</article>
