...
@section('title', 'Testimonials — '.config('travel.brand.name'))
@section('description', 'What travellers say after their trip with '.config('travel.brand.name').'.')

@section('content')

<section class="px-4 pt-11 pb-6.5 sm:px-5">
    <div class="mx-auto max-w-[1400px]">
        <span class="section-kicker">Guest accounts</span>
        <h1 class="mt-4 text-[clamp(34px,4.6vw,64px)] leading-[1] tracking-[-.035em]">Testimonials</h1>
    </div>
</section>
        ...
        @foreach ($testimonials as $quote)
            <div class="glass-strong flex flex-col rounded-[28px] p-7.5">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="rounded-full bg-accent-100 px-3 py-1.5 text-[11.5px] font-bold text-accent-800">{{ $quote['tour'] }}</span>
                </div>
            </div>
        @endforeach
    </div>
