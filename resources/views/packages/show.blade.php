...
<section class="relative -mt-20 flex min-h-[440px] items-end overflow-hidden bg-[#dfe5de] bg-cover bg-center sm:min-h-[560px]"
     style="background-image:url('{{ $package->heroImageUrl(1920, 900) }}')">
    ...
    <div class="relative mx-auto w-full max-w-[1400px] px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
        ...
        <div class="my-4.5 flex flex-wrap gap-2">
            ...
            <span class="rounded-full border border-white/30 bg-white/16 px-3.5 py-1.5 text-xs font-semibold text-white backdrop-blur-md">{{ travel_t($package['location']) }}</span>
            ...
            <span class="rounded-full border border-white/30 bg-white/16 px-3.5 py-1.5 text-xs font-semibold text-white backdrop-blur-md">{{ travel_t($package['pax']) }}</span>
        </div>
        ...
        <div>
            ...
            <div class="glass-strong mb-6 rounded-[30px] p-6 sm:p-9">
                ...
                @foreach ($itinerary as $day)
                    <details class="group mb-2.5 overflow-hidden rounded-[20px] bg-white/55 open:bg-accent-100" @if(! empty($day['open'])) open @endif>
                        <summary class="flex cursor-pointer list-none items-center gap-4 px-5 py-4.5 marker:content-['']">
                            <span class="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-black/5 text-[13px] font-semibold group-open:bg-accent group-open:text-white" style="font-family:var(--font-heading)">{{ $loop->iteration }}</span>
                        ...
                        <div class="px-5 pt-0 pb-5.5 pl-[72px]">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach (['stay', 'meals'] as $tag)
                                    @if (! empty($day[$tag]))
                                        <span class="rounded-full bg-white px-3.5 py-1.5 text-xs font-semibold">{{ travel_t($day[$tag]) }}</span>
                                    @endif
                                @endforeach
                            </div>
                        </div>
...
<div class="sticky bottom-24 z-40 px-4 sm:px-5 lg:bottom-4">
    <div class="mx-auto flex max-w-[1400px] flex-wrap items-center gap-4.5 rounded-full border border-white/16 bg-[rgba(14,32,20,.72)] py-3 pr-5 pl-6.5 backdrop-blur-xl">
        <div class="mr-auto">
            ...
            <div class="mt-0.5 text-xs text-white/62">{{ __('site.packages.sticky_meta', ['days' => $package['days'], 'where' => travel_t($package['location'])]) }}</div>
        </div>
